...
# model is y = w*x + b (linear regression with 4 input features)
NUM_FEATURES = 4
# weight values that generate the data
TRUE_W = [1.5, -2.0, 0.5, 3.0]
TRUE_B = 0.7

def make_dataset(client_id, n_samples):
    # random data generation based on client id
    ...
    for _ in range(n_samples):
        ...
        # compute the target using the weight values
        target = 0.0
        for w, f in zip(TRUE_W, row):
            target = target + w * f
        target = target + TRUE_B

        # adding noise to the target to make it more realistic
...
def subtract(new, base):
    # computing the difference between the new and base weights
    result = {}
    for layer in new:
        diff = []
...
def fedavg(base_weights, updates):
    # avg deltas: weighted by how much data each client had
    total = 0
